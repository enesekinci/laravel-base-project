...
class Product extends Model
{
    ...
    {
        return $this->belongsToMany(Option::class, 'product_variants')
            ->join('variant_option_values', 'product_variants.id', '=', 'variant_option_values.product_variant_id')
            ->join('option_values', 'variant_option_values.option_value_id', '=', 'option_values.id')
            ->select('options.*')
            ->distinct();
    }
    ...
    {
        // Featured products - şimdilik en yeni ve aktif olanların ilk 5'i
        return $query->active()
            ->inStock()
    ...
    {
        // New arrivals - new_from ve new_to tarihlerine göre veya son 30 günde oluşturulanlar
        return $query->active()
            ->inStock()
            ->where(function ($q) {
                $q->where(function ($subQ) {
                    $subQ->whereNotNull('new_from')
                        ->where('new_from', '<=', now())
                        ->where(function ($dateQ) {
                            $dateQ->whereNull('new_to')
                                ->orWhere('new_to', '>=', now());
                        });
    ...
    {
        // Best selling - şimdilik en çok stokta olanlar (order_items tablosu yoksa)
        return $query->active()
            ->inStock()
    ...
    {
        // Top rated - şimdilik en yüksek fiyatlılar (reviews tablosu yoksa)
        // İleride reviews tablosu eklendiğinde bu scope güncellenebilir
        return $query->active()
            ->inStock()
            ->orderBy('price', 'desc')
            ->orderBy('created_at', 'desc');
