                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox
                                ...
                                onCheckedChange={(checked) =>
                                    onUpdate(index, 'is_active', checked === true)
                                }
                            />
                            <Label
                    ...
                    <CollapsibleContent className="mt-2">
                        <div className="space-y-3">
                            {item.children?.map((child, childIndex) => (
                                <MenuItemForm
                                    key={(child as MenuItem & { tempId: string }).tempId}
                                    item={child as MenuItem & { tempId: string }}
                                    index={childIndex}
                                    ...
                                    onRemove={(idx) => {
                                        const updated = (item.children || []).filter(
                                            (_, i) => i !== idx,
                                        );
                                        onUpdate(index, 'children', updated);
                                    }}
...
export default function MenusEdit({ menu }: Props) {
    ...
    useEffect(() => {
        // Menu items'ı yükle ve tempId ekle
        const itemsWithTempId = (menu.items || []).map((item, index) => ({
            ...(item as MenuItem),
            tempId: (item as MenuItem & { tempId?: string }).tempId || `item-${index}-${Date.now()}`,
            children: (item as MenuItem).children?.map((child, childIndex) => ({
                ...child,
                tempId: (child as MenuItem & { tempId?: string }).tempId || `child-${index}-${childIndex}-${Date.now()}`,
            })),
        }));
        setMenuItems(itemsWithTempId as (MenuItem & { tempId: string })[]);
    }, [menu.items]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const cleanItems: MenuItem[] = menuItems.map((item) => {
            ...
            if (cleanItem.children && cleanItem.children.length > 0) {
                result.children = cleanItem.children.map((child) => {
                    // eslint-disable-next-line @typescript-eslint/no-unused-vars
                    const { tempId: childTempId, ...cleanChild } = child as MenuItem & { tempId: string };
                    return {
        ...
        // @ts-expect-error - MenuItem[] type'ı router.put'un beklediği type ile uyumlu değil ama backend'e doğru şekilde gönderiliyor
        router.put(`/admin/menus/${menu.id}`, formData);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${menu.name} - Menü Düzenle`} />
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {menuItems.length === 0 ? (
                                ...
                                >
                                    <SortableContext
                                        items={menuItems.map((item) => item.tempId)}
