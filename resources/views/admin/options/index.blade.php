            ...
            Alpine.data('optionList', () => ({
                ...
                async loadOptions(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/options', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Seçenekler yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadOptions(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Option', 'Bu seçeneği silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/options/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Seçenek silindi')
                                }
                                await this.loadOptions(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="optionList" x-init="init()">
                ...
                <div class="widget-content widget-content-area br-8 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.options.create') }}" class="btn btn-primary">
                            ...
                            <span class="ms-2">New Option</span>
                        </a>
                    </div>
                    ...
                    <div class="table-responsive" x-show="!loading">
                        <table class="table dt-table-hover" style="width: 100%">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="option.name"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span x-text="option.values?.length || 0"></span>
                                        </td>
                                ...
                                <tr x-show="options.length === 0 && !loading">
