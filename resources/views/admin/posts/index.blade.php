            ...
            Alpine.data('postList', () => ({
                ...
                async loadPosts(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/posts', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Blog yazıları yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Post', 'Bu blog yazısını silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/posts/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Blog yazısı silindi')
                                }
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="post.title"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="post.slug"></span>
                                        </td>
                                        <td>
                                            <span :class="post.status === 'published' ? 'badge badge-success' : 'badge badge-warning'" x-text="post.status"></span>
                                        </td>
                                        <td>
                                            <span x-text="post.published_at ? new Date(post.published_at).toLocaleDateString() : '-'"></span>
                                        </td>
