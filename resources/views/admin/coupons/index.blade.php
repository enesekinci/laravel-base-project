            ...
            Alpine.data('couponList', () => ({
                ...
                async loadCoupons(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/coupons', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Kuponlar yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadCoupons(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Coupon', 'Bu kuponu silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/coupons/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Kupon silindi')
                                }
                                await this.loadCoupons(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="couponList" x-init="init()">
                ...
                <div class="widget-content widget-content-area br-8 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.coupons.create') }}" class="btn btn-primary">
                            ...
                            <span class="ms-2">New Coupon</span>
                        </a>
                    </div>
                    ...
                    <div class="widget-content widget-content-area br-8 p-4 mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="text" x-model="filters.search" @input.debounce.500ms="applyFilters()" placeholder="Code..." class="form-control" />
                            </div>
                            <div class="col-md-4">
                    ...
                    <div class="table-responsive" x-show="!loading">
                        <table class="table dt-table-hover" style="width: 100%">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="coupon.code"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge badge-info" x-text="coupon.type"></span>
                                        </td>
                                        <td>
                                            <span x-text="coupon.type === 'percent' ? `${coupon.value}%` : `₺${coupon.value}`"></span>
                                        </td>
                                        <td>
                                            <span x-text="coupon.start_date ? new Date(coupon.start_date).toLocaleDateString() : '-'"></span>
                                        </td>
                                        <td>
                                            <span x-text="coupon.end_date ? new Date(coupon.end_date).toLocaleDateString() : '-'"></span>
                                        </td>
                                        <td>
                                            <span :class="coupon.is_active ? 'badge badge-success' : 'badge badge-danger'" x-text="coupon.is_active ? 'Active' : 'Inactive'"></span>
                                        </td>
                                ...
                                <tr x-show="coupons.length === 0 && !loading">
