            ...
            Alpine.data('pageList', () => ({
                ...
                async loadPages(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/pages', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Sayfalar yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Page', 'Bu sayfayı silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/pages/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Sayfa silindi')
                                }
                                await this.loadPages(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="pageList" x-init="init()">
                    ...
                    <div class="widget-content widget-content-area br-8 p-4 mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="text" x-model="filters.search" @input.debounce.500ms="applyFilters()" placeholder="Title or slug..." class="form-control" />
                            </div>
                            <div class="col-md-4">
                    ...
                    <div class="table-responsive" x-show="!loading">
                        <table class="table dt-table-hover" style="width: 100%">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="page.title"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="page.slug"></span>
                                        </td>
                                        <td>
                                            <span :class="page.status === 'published' ? 'badge badge-success' : 'badge badge-warning'" x-text="page.status"></span>
                                        </td>
                                        <td>
                                            <span x-text="page.created_at ? new Date(page.created_at).toLocaleDateString() : '-'"></span>
                                        </td>
                                ...
                                <tr x-show="pages.length === 0 && !loading">
