            ...
            Alpine.data('postTagList', () => ({
                ...
                async loadTags(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/post-tags', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Blog etiketleri yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Tag', 'Bu blog etiketini silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/post-tags/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Blog etiketi silindi')
                                }
...
@section('content')
    ...
    <div x-data="postTagList" x-init="init()">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a
                                                    :href="`/admin/post-tags/${tag.id}/edit`"
