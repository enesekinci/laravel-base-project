...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('shippingMethodList', () => ({
                shippingMethods: [],
                loading: false,
                filters: {
                    search: '',
                ...
                async loadShippingMethods(page = 1) {
                    if (!window.adminApiHelpers) {
                        console.error('adminApiHelpers is not available')
                        return
                    }
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/shipping-methods', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Kargo yöntemleri yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async applyFilters() {
                    ...
                    await this.loadShippingMethods(1)
                },
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadShippingMethods(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Shipping Method', 'Bu kargo yöntemini silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/shipping-methods/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Kargo yöntemi silindi')
                                }
                                await this.loadShippingMethods(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="shippingMethodList" x-init="init()">
                ...
                <div class="widget-content widget-content-area br-8 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.shipping-methods.create') }}" class="btn btn-primary">
                            ...
                            <span class="ms-2">New Shipping Method</span>
                        </a>
                    </div>
                            ...
                            <tbody>
                                <template x-for="method in shippingMethods" :key="method.id">
                                    <tr>
                                        ...
                                        <td>
                                            <span :class="method.is_active ? 'badge badge-success' : 'badge badge-danger'" x-text="method.is_active ? 'Active' : 'Inactive'"></span>
                                        </td>
                                ...
                                <tr x-show="shippingMethods.length === 0 && !loading">
                                    <td colspan="4" class="text-center">
                                        <p class="text-muted">No shipping methods found</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
