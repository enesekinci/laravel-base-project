            ...
            Alpine.data('brandList', () => ({
                ...
                async loadBrands(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/brands', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Markalar yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadBrands(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Brand', 'Bu markayı silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/brands/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Marka silindi')
                                }
                                await this.loadBrands(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="brandList" x-init="init()">
                ...
                <div class="widget-content widget-content-area br-8 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.brands.create') }}" class="btn btn-primary">
                            ...
                            <span class="ms-2">New Brand</span>
                        </a>
                    </div>
                    ...
                    <div class="widget-content widget-content-area br-8 p-4 mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="text" x-model="filters.search" @input.debounce.500ms="applyFilters()" placeholder="Name or slug..." class="form-control" />
                            </div>
                            <div class="col-md-4">
                            ...
                            <tbody>
                                <template x-for="brand in brands" :key="brand.id">
                                    <tr>
                                        <td>
                                            <div class="avatar">
                                                <img :src="brand.logo_url || '/placeholder-image.png'" :alt="brand.name" style="width: 64px; height: 64px; object-fit: cover; border-radius: 8px" />
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="brand.name"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="brand.slug"></span>
                                        </td>
                                        <td>
                                            <span :class="brand.is_active ? 'badge badge-success' : 'badge badge-danger'" x-text="brand.is_active ? 'Active' : 'Inactive'"></span>
                                        </td>
                                ...
                                <tr x-show="brands.length === 0 && !loading">
