            ...
            Alpine.data('postCategoryList', () => ({
                ...
                async loadCategories(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/post-categories', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Blog kategorileri yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Category', 'Bu blog kategorisini silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/post-categories/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Blog kategorisi silindi')
                                }
...
@section('content')
    ...
    <div x-data="postCategoryList" x-init="init()">
