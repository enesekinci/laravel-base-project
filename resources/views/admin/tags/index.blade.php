            ...
            Alpine.data('tagList', () => ({
                ...
                async loadTags(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/tags', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Etiketler yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadTags(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Tag', 'Bu etiketi silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/tags/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Etiket silindi')
                                }
                                await this.loadTags(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="tagList" x-init="init()">
                    ...
                    <div class="table-responsive" x-show="!loading">
                        <table class="table dt-table-hover" style="width: 100%">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="tag.name"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="tag.slug"></span>
                                        </td>
                                ...
                                <tr x-show="tags.length === 0 && !loading">
