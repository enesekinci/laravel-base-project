            ...
            Alpine.data('menuList', () => ({
                ...
                async loadMenus(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/menus', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Menüler yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadMenus(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Menu', 'Bu menüyü silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/menus/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Menü silindi')
                                }
                                await this.loadMenus(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="menuList" x-init="init()">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="menu.name"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="menu.location || '-'"></span>
                                        </td>
                                        <td>
                                            <span x-text="menu.items?.length || 0"></span>
                                        </td>
