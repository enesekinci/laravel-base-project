            ...
            Alpine.data('attributeList', () => ({
                ...
                async loadAttributes(page = 1) {
                    ...
                    try {
                        ...
                        const response = await window.adminApiHelpers.get('/attributes', params)
                        ...
                        if (window.Alpine && window.Alpine.store('toast')) {
                            window.Alpine.store('toast').error('Özellikler yüklenirken bir hata oluştu')
                        }
                    } finally {
                        this.loading = false
                    }
                ...
                async changePage(page) {
                    if (page >= 1 && page <= this.pagination.last_page) {
                        await this.loadAttributes(page)
                    }
                },

                async deleteItem(id) {
                    if (window.Alpine && window.Alpine.store('confirm')) {
                        window.Alpine.store('confirm').show('Delete Attribute', 'Bu özelliği silmek istediğinize emin misiniz?', async () => {
                            try {
                                await window.adminApiHelpers.delete(`/attributes/${id}`)
                                if (window.Alpine && window.Alpine.store('toast')) {
                                    window.Alpine.store('toast').success('Özellik silindi')
                                }
                                await this.loadAttributes(this.pagination.current_page)
                            } catch (error) {
...
@section('content')
    ...
    <div x-data="attributeList" x-init="init()">
                ...
                <div class="widget-content widget-content-area br-8 p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.attributes.create') }}" class="btn btn-primary">
                            ...
                            <span class="ms-2">New Attribute</span>
                        </a>
                    </div>
                    ...
                    <div class="widget-content widget-content-area br-8 p-4 mb-4">
                        <div class="row">
                            <div class="col-md-10">
                                ...
                                <input type="text" x-model="filters.search" @input.debounce.500ms="applyFilters()" placeholder="Name or code..." class="form-control" />
                            </div>
                    ...
                    <div class="table-responsive" x-show="!loading">
                        <table class="table dt-table-hover" style="width: 100%">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <span class="text-truncate fw-bold" x-text="attribute.name"></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="text-truncate" x-text="attribute.code"></span>
                                        </td>
                                        <td>
                                            <span class="badge badge-info" x-text="attribute.type || 'text'"></span>
                                        </td>
                                ...
                                <tr x-show="attributes.length === 0 && !loading">
